...
' SEC-09 newrisk。カード形式。0件のときは何も描かず、登録表の empty:'note' が
...
Public Function SecNewRiskJs() As String
    ...
    s = s & "var card=T(el,'div','card card-deep');" & vbLf
    s = s & "T(card,'h3',null,S(x.risk_name));" & vbLf
    s = s & "var c=T(card,'div','chips');" & vbLf
    s = s & "CHIP(c,LB(LCAT,x.category));" & vbLf
    s = s & "CHIP(c,'時間軸 '+LB(LHZ,x.horizon),'chip-ai');" & vbLf
    s = s & "T(card,'p',null,S(x.scenario));" & vbLf
    s = s & "if(NB(x.evidence_quote)){T(card,'p','muted','根拠: 「'+S(x.evidence_quote)" & vbLf
    s = s & "+'」('+LB(LSRC,x.evidence_source)+')');}" & vbLf
    s = s & "if(NB(x.proposal_hint)){" & vbLf
    s = s & "T(card,'p',null,'提案の糸口: '+S(x.proposal_hint));}}}" & vbLf
    SecNewRiskJs = s
End Function
...
' SEC-10 story。カード3枚(18章§3)。target_risk_nos / target_gap_nos は
'   s2 側を引いて名称に解決する(番号だけを見せない)。
Public Function SecStoryJs() As String
    ...
    s = s & "var c=T(card,'div','chips');" & vbLf
    s = s & "CHIP(c,'テーマ'+S(y.story_no),'chip-ai');" & vbLf
