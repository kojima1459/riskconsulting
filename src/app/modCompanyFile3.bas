...
' ============================================================================
' CaseSheetCols - dossier_case シートに書く列の並び(写像。13章§2.8 v2.8)。
'   案件一覧の全列 + schema_version + pii_flag。**この1本が dossier_case の
'   列の正**で、WriteCaseRow はこの並びだけを見て書く(列を足すときはここ)。
'   pii_flag は統合W10の裁定: 自分の data_dir への自動保存は PII 検知があっても
'   行う(自分の保管場所への保存は「共有・送信」ではない)ため、代わりに
'   **印を残す**列。16章 E-05(7)。
' ============================================================================
Public Function CaseSheetCols() As String
    CaseSheetCols = CF3_CASE_COLS & CF3_SEP & "schema_version" & CF3_SEP & "pii_flag"
End Function

' ============================================================================
' PiiFlagOf - PII走査の結果テキスト -> pii_flag の値(写像。16章 E-05(7))。
'   検知あり(走査結果が空でない) -> "TRUE" / 検知なし -> "FALSE"。
'   前後の空白だけの文字列は「検知なし」として扱う。
' ============================================================================
Public Function PiiFlagOf(ByVal piiText As String) As String
    If LenB(Trim$(piiText)) = 0 Then
        PiiFlagOf = "FALSE"
    Else
        PiiFlagOf = "TRUE"
    End If
...
' ----------------------------------------------------------------------------
'   戻り値: CF3_SAVED   書き出した
'           CF3_SKIPPED 書き出さなかった(案件未選択・保存先不明)
'           CF3_FAILED  書き出そうとして失敗した(ui層が toast と err_log を出す)
'
'   **PII検知があっても書き出す**(統合W10の裁定)。16章 E-05(7) は共有・送信・
'   配布に対する規定であり、自分の data_dir への自動保存は対象外。検知の事実は
'   dossier_case の pii_flag(TRUE/FALSE)に印として残す。
'   利用者へは何も出さない(無言の自動保存)。
' ============================================================================
Public Function AutoSaveCase(ByVal caseId As String) As String
    ...
    ' 統合W10の裁定: PII検知があっても**自分の OneDrive(data_dir)への自動保存は
    '   行う**(自分の保管場所への保存は「共有・送信」ではない。16章 E-05(7))。
    '   代わりに dossier_case の pii_flag に印を残す(WriteCaseRow)。
    Dim pathText As String
    pathText = modCompanyFile.ExportCompanyFile(caseId, dirText, vbNullString, True)
...
Private Sub WriteCaseRow(ByVal wb As Object, ByVal caseId As String)
    ...
    Dim cols() As String
    cols = modUtil.SplitKeepNonEmpty(CaseSheetCols(), CF3_SEP)
    Dim i As Long
    For i = LBound(cols) To UBound(cols)
        Select Case cols(i)
        Case "schema_version"
            modCompanyFile2.SheetPutText ws, hdr, wr, cols(i), CF3_SCHEMA
        Case "pii_flag"
            ' 統合W10: 自動保存は PII があっても止めないので、印だけ残す。
            modCompanyFile2.SheetPutText ws, hdr, wr, cols(i), _
                PiiFlagOf(modCompanyFile.ScanCaseForPii(caseId))
        Case Else
            modCompanyFile2.SheetPutText ws, hdr, wr, cols(i), _
