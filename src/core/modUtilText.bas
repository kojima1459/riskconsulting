...
'   tailPart : 用途を表す末尾(例 "企業カルテ" / "リスクレポート_20260901")
...
' ----------------------------------------------------------------------------
'   適用順の正は18章§5.3(1)。この順を守らないと二重エスケープになる。
'     1. \ -> \\ ・ " -> \"
'     2. < -> \u003C       (**すべての < が対象**。18章§5.3(1) v1.1改訂)
'     3. U+2028 -> \u2028 ・ U+2029 -> \u2029 (JSでは行終端子として扱われる)
'     4. その他の制御文字(U+0000-U+001F) -> \u00XX
'   1を最初に置くのは、後段で足す "\" が再エスケープされないようにするため。
'   2が「</ だけ」ではなく「< 全部」なのは、HTMLトークナイザが <script> の中身で
'     <!-- を見ると script data escaped 状態へ、続く <script で double escaped 状態へ
'     入り、そこでは </script> が終端として働かなくなるため。旧規約(</ -> <\/)では
'     <!--<script> を含むLLM出力1件でDATAブロックの正規の </script> が食われ、
'     後続のランタイムJSごと飲み込まれてページが白紙化した(16章E-47・W3.1裁定)。
'     < を1文字残らず落とせば </script> も <!-- も <script も構造上現れない。
'   4は改行・タブも含めて \u00XX にする(JSON.parse が \u003C を < へ、\u000A を
'   改行へ戻すので情報は落ちない)。
' ============================================================================
