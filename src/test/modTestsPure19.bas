...
' ============================================================================
' modTestsPure19 - W10(データをブックの外へ・裁定書28)の純層テスト(17章§4-1 層(a))
' ----------------------------------------------------------------------------
...
Private Const P19_BAD As String = "\/:*?""<>|"
' 13章§2.8 の命名テンプレートの末尾(拡張子込み)。
Private Const P19_TAIL As String = "_企業カルテ.xlsx"
...
Private Sub T_W10F_CompanyFileName()
    ...
    Dim i As Long
    For i = 1 To Len(P19_BAD)
        If InStr(1, nameText, Mid$(P19_BAD, i, 1), vbBinaryCompare) > 0 Then leftOver = True
    Next i

    ChkT "Test_W10F_01_禁止文字が名前に残らず末尾は企業カルテ_13章§2.8手順1", _
        (Not leftOver) And (Len(nameText) > Len(P19_TAIL)) And _
        (Right$(nameText, Len(P19_TAIL)) = P19_TAIL), _
        "実際=[" & nameText & "]"
