...
Public Function BuildS4Json() As String
    ...
    s = s & "{""file_title"":""株式会社浜松スイーツファクトリー様 リスクマネジメントのご提案(骨子)"",""slides"":[{""slide_no"":1,""title"":""貴社の事業環境の理解"",""bullets"":[""浜松2工場体制で焼き菓子を一貫生産"",""EC直販比率の拡大を経営方針として推進中"",""国内菓子市場は縮小基調もギフト需要とEC市場は堅調""],""notes"":""まず御社を調べてきたことが伝わる事実整理から入ります""},{""slide_no"":2,""title"":""潜在リスクの全体像"",""bullets"":[""アレルゲン表示誤りによる自主回収リスク"",""浸水による工場操業停止リスク"",""ECサイト個人情報漏えいリスク"",""原料調達難による生産停滞リスク""],""notes"":""影響度と発生しやすさで整理して見せます""},{""slide_no"":3,""title"":""同業種で見られる事故・トラブルの類型"",""bullets"":[""食品製造業でのアレルゲン混入による自主回収の類型が知られている"",""水災による工場操業停止の類型が知られている""" & vbLf
    s = s & ",""ECサイトからの個人情報漏えいの類型が知られている""],""notes"":""実在の個別事故の社名・数値は出さず一般的な類型として話します""},{""slide_no"":4,""title"":""当社がご支援できること"",""bullets"":[""PL保険とリコール費用特約の拡充提案"",""サイバー保険の新規提案"",""検知×補償バンドル型による水濡れ対策提案""],""notes"":""提案ストーリー3本をメニュー名・型を使って紹介します""},{""slide_no"":5,""title"":""次のステップ"",""bullets"":[""詳細診断のご提案"",""伺いたい事項の予告""],""notes"":""次回訪問でヒアリングしたい項目を予告します""}],""hearing_questions"":[{""question"":""浜松本社工場の建物構造(耐火・耐震等級)について教えていただけますか"",""purpose"":""施設・自然災害リスクの評価のため""},{""question"":""EC物流の委託先は何社に分散していますか"",""purpose"":""サプライチェーンリスクの評価のため""" & vbLf
    s = s & "},{""question"":""直近で食品衛生に関する指摘や自主回収はありましたか"",""purpose"":""製造・品質リスクの評価のため""},{""question"":""止水板などの水災対策は導入されていますか"",""purpose"":""施設・自然災害リスクの確認のため""},{""question"":""ECサイトの個人情報保護体制はどの程度整備されていますか"",""purpose"":""デジタル・情報リスクの確認のため""},{""question"":""パート従業員の労災上乗せ補償の加入状況を教えてください"",""purpose"":""人材・労務リスクの確認のため""},{""question"":""主要原材料の仕入先は複数確保されていますか"",""purpose"":""調達・供給網リスクの確認のため""},{""question"":""現在のPL保険にリコール費用特約は付帯していますか"",""purpose"":""付保ギャップの確認のため""}]}"
...
Public Function BuildPfJson() As String
    ...
    s = s & ",""ok"":false}],""grammar_checks"":[{""key"":""a"",""label"":""既存アセットに載る"",""ok"":true,""note"":""検知パートナー連携の枠組みは既存の型ライブラリに近い""},{""key"":""b"",""label"":""引受判断に変換されている"",""ok"":false,""note"":""トリガーの定義がまだ曖昧で引受判断に落とし込めていない""},{""key"":""c"",""label"":""当社の支払データで損害が語れる"",""ok"":false,""note"":""配送遅延の支払データが社内に無い""},{""key"":""d"",""label"":""保険料を払う法人・自治体が特定できる"",""ok"":true,""note"":""菓子製造企業が契約者になる想定は明確""}],""duplicates"":[{""ref_id"":""M-0012"",""relation"":""近接"",""note"":""検知×診断サービスの発想が既存メニューと近い""}],""predicted_drop_types"":[""T4"",""T9""],""rework_suggestions""" & vbLf
    s = s & ":[{""approach"":""entry_path"",""pattern_id"":""P9"",""suggestion"":""個人向けではなく卸先の菓子問屋を契約者とする経路に組み替える""},{""approach"":""benefit_form"",""pattern_id"":"""",""suggestion"":""金銭給付ではなく代替配送の現物給付に組み替えて偶然性を明確にする""}],""survival"":""mid"",""advice_to_poster"":""着眼点は良いので、まずトリガーを配送遅延の記録データで定義し直すところから磨きましょう""}"
...
Public Function BuildS2CHitJson() As String
    ...
    s = s & "{""verdict_summary"":""見落としは少ないが根拠の質と移転可能性の判定に改善余地がある"",""issues"":[{""target"":""overall"",""issue_type"":""missing"",""detail"":""営業の現場メモにあるEC物流の外部委託先1社依存という手がかりがリスクとして拾われていない"",""suggestion"":""サプライチェーンのカテゴリで単一委託先依存のリスクを追加してほしい""},{""target"":""risk_no:6"",""issue_type"":""generic"",""detail"":""国内菓子市場の縮小というリスクが業種名を変えても通用する一般論になっている"",""suggestion"":""当社固有の商品構成やEC比率に結びつけて具体化してほしい""},{""target"":""risk_no:3"",""issue_type"":""insurability_error"",""detail"":""浸水リスクをcoverと判定しているが地震保険は別立てであり移転可能性の説明が不十分"",""suggestion"":""transferabilityの根拠に種目ごとの適用範囲を明記してほしい""" & vbLf
...
Public Function BuildS3CHitJson() As String
    ...
    s = s & "{""executive_reactions"":[{""story_no"":1,""reaction"":""PL保険は入っているつもりだったが、リコール費用の話は初耳だ"",""lands"":true},{""story_no"":2,""reaction"":""サイバーは大手が狙われる話でしょう、うちには関係ないのでは"",""lands"":false},{""story_no"":3,""reaction"":""止水板は入れているから、それ以上は今は要らないかな"",""lands"":true}],""issues"":[{""target"":""story_no:2"",""issue_type"":""wont_land"",""detail"":""中小企業を狙うサイバー攻撃の実例が無いと経営者に響かない"",""suggestion"":""同規模企業の被害実例を1件添えて具体性を出す""},{""target"":""overall"",""issue_type"":""uw_concern"",""detail"":""D&Oを見送った判断は妥当だが根拠が投函上に薄い"",""suggestion"":""do_not_proposeの理由を係争有無の確認結果とともに明記する""" & vbLf
    s = s & "}]}"
