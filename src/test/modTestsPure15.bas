...
' ============================================================================
' modTestsPure15 - W7(裁定書25 センターピン整合)の純層テスト(17章§4-1 層(a))
' ----------------------------------------------------------------------------
...
'
' 対象と根拠:
'   W7A FmtIncidents      4本  13章§3.11・15章§3 の整形例(1行の書式 / 空欄の
'                              項目を丸ごと省く / 0行の既定文言 / 非配列でも続行)
'   W7B focus_line_ids    8本  13章§2.1(NormalizeStoryNos 4本＝正常・空白と重複・
'                              非整数の除去・空／CollectLineIds 4本＝採用storyだけ・
'                              重複の統合・該当なし・空の採用番号)
'   W7C CoverageNoteOf    4本  13章§2.11(d)(節の本文だけを返す / 節が空なら空文字 /
'                              例文行は落とす / 見出しが無ければ空文字)
'   計 16本
'
...
WC:
    On Error GoTo FC
    T_W7C_CoverageNoteOf
WDone:
    Exit Sub
FA:
    GroupFail "W7A FmtIncidents"
    Resume WB
FB:
    GroupFail "W7B focus_line_ids"
    Resume WC
FC:
    GroupFail "W7C CoverageNoteOf"
    Resume WDone
End Sub
...
Private Sub T_W7A_FmtIncidents()
    ...
    ChkS "Test_W7A_01_FmtIncidentsの1行の書式_15章§3", _
    ...
    rws = MkRows(HdrInc(), d1)
    ChkS "Test_W7A_02_FmtIncidentsは空欄の項目を丸ごと省く_13章§3.11", _
        modKnowledgeFmt.FmtIncidents(rws), e1

    rws = MkRows(HdrInc(), "")
    ChkS "Test_W7A_03_FmtIncidentsの0行は業種専用の文言_15章§3", _
        modKnowledgeFmt.FmtIncidents(rws), "(この業種の登録事例はまだありません)"

    ChkS "Test_W7A_04_FmtIncidentsは配列でない入力でも既定文言で続行_16章E09", _
        modKnowledgeFmt.FmtIncidents(Empty), "(この業種の登録事例はまだありません)"
End Sub

' ============================================================================
' W7B focus_line_ids の導出(13章§2.1・裁定書25 S4)
' ----------------------------------------------------------------------------
'   used_proposals(`1;3`) -> adopted_story_nos -> 採用storyの line_ids。
'   実在チェック(ExistingLineIds)はナレッジを読むので層(a)では扱わない。
' ============================================================================
Private Sub T_W7B_FocusLineIds()
    ChkS "Test_W7B_01_NormalizeStoryNosは13章の例1;3をそのまま通す_13章§2.1", _
        modCaseStore3.NormalizeStoryNos("1;3"), "1;3"
    ChkS "Test_W7B_02_NormalizeStoryNosは前後の空白と重複を落とす_13章§2.1", _
        modCaseStore3.NormalizeStoryNos(" 2 ; 1 ;2"), "2;1"
    ChkS "Test_W7B_03_NormalizeStoryNosは整数でない要素を捨てる_13章§2.1", _
        modCaseStore3.NormalizeStoryNos("1;採用;3番;0;2"), "1;2"
    ChkS "Test_W7B_04_NormalizeStoryNosは空入力で空文字_13章§2.1", _
    ...
    ChkS "Test_W7B_05_CollectLineIdsは採用storyの種目だけを集める_13章§2.1", _
        modCaseStore3.CollectLineIds(s3, "1"), "L-03;L-07"
    ChkS "Test_W7B_06_CollectLineIdsは複数storyの重複を1本に畳む_13章§2.1", _
        modCaseStore3.CollectLineIds(s3, "1;3"), "L-03;L-07;L-21"
    ChkS "Test_W7B_07_CollectLineIdsは採用番号が空なら空文字_13章§2.1", _
        modCaseStore3.CollectLineIds(s3, ""), ""
    ChkS "Test_W7B_08_CollectLineIdsは該当storyが無ければ空文字_13章§2.1", _
        modCaseStore3.CollectLineIds(s3, "9"), ""
End Sub

' ============================================================================
' W7C CoverageNoteOf(13章§2.11(d)・裁定書25 S1 の二重保存の派生側)
' ============================================================================
Private Sub T_W7C_CoverageNoteOf()
    Dim memo As String
    memo = "【営業メモ】" & vbLf & "社長はワンマン。" & vbLf & _
           "【前回更新メモ】" & vbLf & "地震は見送り。" & vbLf & _
           "【付保の見立て】" & vbLf & "たぶん火災は他社さん。" & vbLf & _
           "労災上乗せは元請の包括に乗っている模様。"
    ChkS "Test_W7C_01_CoverageNoteOfは付保の見立ての本文だけを返す_13章§2.11d", _
    ...
    memo = "【営業メモ】" & vbLf & "社長はワンマン。" & vbLf & _
           ...
           "裏の川が気になる。"
    ChkS "Test_W7C_02_CoverageNoteOfは節が空なら空文字_13章§2.11d", _
        modNavText.CoverageNoteOf(memo), ""

    memo = "【付保の見立て】" & vbLf & _
           "例: たぶん火災は他社さん。労災上乗せは元請の包括に乗っている模様。"
    ChkS "Test_W7C_03_CoverageNoteOfは先置きの例文行を落とす_11章§3.3.3b", _
        modNavText.CoverageNoteOf(memo), ""

    ChkS "Test_W7C_04_CoverageNoteOfは見出しが無ければ空文字_13章§2.11d", _
