# 20. 提案書Wideテンプレート仕様 v1.2

**v1.2（W15 Round 2 検収・裁定書40）の変更**: §3 の機械置換の規約(2)に**サ変語幹**（`suru` 印）を足した（S-M2。「保険化する」が「保険での備え方の設計する」に壊れるため、「〜する」の直前だけ置換しない）。置換しても顧客向け本文に残った社内語は **V-S5-12 の警告として usage_log に残す**（S-M1）。`modPipeline5.SoftenOrFail` は**置換後の CheckS5 に通らなければ必ず失敗**へ戻した（S-M4。16章 E-71③の逐語）。表紙の5項目は coverFields へ入れる前に TAB/LF/CR を落とす（Q-M1。レポート側 R2-05 と同型の穴が提案書側に残っていた）。§7 の発表者ノートの描画を**描画例外の catch の内側**へ入れた（S-M3）。§10 の検査に「EP_S5_REQUIRED と15章§5.6 required の一致」「発表者ノートを壊しても22枚が描かれ印が立つこと」を足し、純テストの本数申告を実数（61本）に直した。
...
- **S2 由来の文字列は、DATAへ入れる前に `modValidate4.SoftenTaboo` で対訳表の機械置換を通す**。S2 は営業向けの語彙で書かれており、LLM の言い換えを経ずにお客さまの目に触れる唯一の経路であるため。置換は次の4規約に従う（**W15 最後の実装波・裁定書43 §1 で設計を変えた**。正は対訳表§6〜§6.6）:
  ...
  2. **置換してよい文脈を閉じた集合で定義する**（対訳表§6 の**終端集合**）。社内語 W を置換してよいのは、**W の直後の1文字が終端集合（助詞 `をにはがのへとでもやかねよら`・記号・空白・改行・タブの61字）に入るとき、または W が文字列の末尾のとき**だけ。それ以外（直後が漢字・ひらがな・カタカナ・英数）は置換しない。終端でない位置では**より短い語も当てない**（その位置の走査を打ち切る）。前の設計は「置換を見送る語尾」を印ごとの白名簿で列挙しており、3ラウンドにわたって「3件直すと同じ型が3件残る」（整理ている→整理ながら／会社役員賠償責任保険保険→額額）を繰り返した。活用語尾は開いた集合であり、**列挙では閉じない**。
  3. **`mode=warn` の対は一切置換しない**（対訳表§6.1）。第3列は `replace` / `warn` の**2区分だけ**で、旧 `general` / `suru` / `verb` の3印はここへ集約した。`warn` は一般語（移転・保有・抜け）・顧客語が述語（未充足）・動作性名詞で顧客語が静的名詞句（ヒアリング・顕在化・与信・クロスセル）・用言の連用形（仕分け・引受け）の**全10対**。置換しない語も **V-S5-12 の警告には出す**（置換しないことと見逃すことは別）。**置換しなかった社内語が顧客向け本文に残ったら、`GenerateProposalHtml` が usage_log へ `proposal_taboo_left` / `proposal_jargon_left=<件数>` と V-S5-12 の警告文を残す**（裁定書40 S-M1・43 §1-4。警告なので生成は止めず、営業向けレポートには出さない）。S5 の経路（`modPipeline5.SoftenOrFail`）では扱いが違い、**置換後の `CheckS5` に1件でも発火が残れば失敗**である（16章 E-71③「通れば成功として続行する」。裁定書40 S-M4）。
  4. **冪等**（置換結果をもう一度通しても1件も変わらない。顧客語が別の社内語を含む対があるため変化が無くなるまで通す）と、**取り消し規則**（対訳表§6.5。継ぎ目で同じ文字が3つ以上続く／顧客語の末尾1〜3文字が直後と重複するなら、その置換を取り消す二重の安全網）。
...
- **描画が失敗した枚の扱い**（W15 Round2 R2-12）: 描画関数が例外を投げた枚も同じ1行に置き換わるが、それだけでは「わざと保留した項目」と見分けが付かない。`modProposalHtml1` の `run()` は `catch` で **`section` に `data-render-error="1"` を付ける**。この印は**既定では見た目を変えない**（お客さまの画面は1ピクセルも変わらない）。社内で確認するときだけ URL に `?debug=1` を付けると `body.debug` が付き、その枚に赤枠（`outline`）が出る（§11）。**上流の防壁は §1.1②' の必須キー検査**であり、この印は最後の保険である。印を立てる `catch` は本文の描画（`d.render`）と発表者ノートの描画（`notes()`。§7）の**両方**を包む。
...
- **ノートの描画も、本文の描画と同じ `catch` の内側で行う**（W15 Round2 検収・裁定書40 S-M3）。ノートの値源は S5 の `notes[]`（LLM出力であり `s5_edited` を人が直す経路もある）なので、ここで例外が飛ぶと `run()` ごと止まり、**提案書が1枚も描かれない白紙**になる。例外が出た枚は §4.1 と同じ `data-render-error="1"` を立てて次の枚へ進む。§10 の検査13がこれを実測する。
...
- `tools/render_proposal.py`（`render_report.py` と同型。gate.py への登録は司令塔が行う）。mock の S5 JSON（`MK-S5`）と S2 JSON（`MK-S2-RNW`）から実物のHTMLを組み、次を機械で見る。
  ...
  12. `modExportProposal` の `EP_S5_REQUIRED` が15章§5.6 Schema-S5 の最外 `required` と**逐語で一致**する（W15 Round2 検収・裁定書40 S-m。required の写しが3箇所あるのに、15章⇔`modSchemas2` しか照合されておらず、必須キー検査(E0502)だけが古い写しのまま取り残されうる）
  13. **発表者ノートの素材を壊しても**（`DATA.p.notes=[null]`）22枚が描かれ、壊れた枚に `data-render-error` が立つ（裁定書40 S-M3）。素の素材では印が1枚も立たない
  14. **宣言⇔実装の全列突合**（`check_glossary_impl`）: 対訳表（§1〜§3 の番号付き46語＋§4.1 の代替語3語＋§6.4 の表記ゆれ2語＝**51対**）の社内語・顧客語・`mode` が `modValidate4.TabooPairs()` の実物と**1行ずつ一致**し、§6 の**終端集合61字が実装の `TermChars()` と1文字ずつ一致**し、`mode` の語彙が `replace` / `warn` の2語だけで、§6.5 の取り消し規則の字数が実装の定数と一致すること（列を1つ足したら宣言と実装の両方に要る）
  15. **壊れの徴候が1件も無いこと**（`check_glossary_effective`）: LibreOffice で**実物の `modValidate4.SoftenTaboo` を回し**、51対 × 全文脈（終端集合の各文字・非終端の代表文字（漢字/ひらがな/カタカナ/英数）・活用語尾）の**実効出力 9,180件**に (a) 同一文字3連続 (b) 顧客語の末尾1〜3文字と直後の重複 (c) 置換が起きたのに直後が終端集合でない がどれも無く、対訳表の宣言だけから組み立てた参照実装の出力と1件ずつ一致し、2回通しても変わらない（冪等）こと。**入力した文脈が1件残らず採れていること**も見る（採取が欠けた分だけ検査が静かに減るのを防ぐ）
- 14 と 15 は W15 最終是正（裁定書42 §1）で足し、**裁定書43 §1-5 で作り直した**。42 の版は「対の数と一部」しか見ておらず、実装の語尾リストを半分に削っても全ゲートが緑だった。値源（対訳表 Markdown）と実装（VBA）を別々に辿るので、どちらか一方だけが変わると必ず赤くなる。要点行には**実際に検査した対の数・終端集合の字数・実効出力の件数**を出す。
- 純テスト `src/test/modTestsPure27.bas` と `src/test/modTestsPure28.bas`（層(a)）。28 は 27 が30,000字の上限に達したための続きで、**裁定書43 §1 の設計変更の20本**——終端集合の文脈では置換する4本（助詞・記号・文字列の末尾・全角空白）・終端でない文脈では置換しない5本（ひらがな／漢字「PML額」／カタカナ／英数／複合語「D&O保険」「BCP計画」）・`mode=warn` は一切置換しない4本（一般語／顧客語が述語／動作性名詞で顧客語が静的名詞句／用言の連用形）・最長一致と表記ゆれ2本・対訳表そのもの5本（51行で warn は10行／mode は2語だけ／`TabooHitStrict` の両方向／冪等）——を持つ。**42 の版は「PML額 → 想定最大損害額額」「D&O保険 → 会社役員賠償責任保険」という壊れた出力を正解として固定していた**（裁定書43 §1-6）。27 は**合計61本**（G1=14 / G2=5 / G3=4 / G4=3 / G5=16 / G6=19。本数は `wintest/tests_expected.txt` の `prod` と必ず同時に直す）。CheckS5 の13件・登録表の枚数・空データで落ちないこと・内部値がDATAに入らないこと・`reviewedBy` 空で生成しないこと・ファイル名規則・W15 Round 2 の16本（裁定書39）と検収の19本（裁定書40）。裁定書43 §1 で期待値が変わったのは、`TabooHitStrict` の意味（置換の取りこぼし＝終端の文脈に残った `replace` の語だけ）と、用言の連用形を `warn` にしたことの2点である。
