...
- **S2 由来の文字列は、DATAへ入れる前に `modValidate4.SoftenTaboo` で対訳表の機械置換を通す**。S2 は営業向けの語彙で書かれており、LLM の言い換えを経ずにお客さまの目に触れる唯一の経路であるため。置換は次の3規約に従う（W15 Round2 R2-02）:
  ...
  2. **印のある語は（その位置では）置換しない**。対訳表 §6 の第3列の印を持つ行が対象で、印は2種類ある（W15 Round2 検収・裁定書40 S-M2）。
     - `general`（一般語）… 「移転」「保有」「抜け」は社内語であると同時に日常語であり（本社を移転する／現金を保有する）、機械が潰すと「本社を保険で備えるする」になる。**常に**置換しない。
     - `suru`（サ変語幹）… 「付保」「保険化」「特約開発」「組成」「引受」「ヒアリング」「攻めの保険活用」「顕在化」「与信」「クロスセル」は「〜する」に続けて使う。顧客語が名詞句なので、置換すると「保険化する」→「保険での備え方の設計する」のように壊れる。**直後が `し` / `す` / `さ` / `せ` のときだけ**置換せず、名詞として使われている「保険化の検討」は従来どおり置換する。
     どちらの印も **V-S5-12 の警告には出す**（置換しないことと見逃すことは別）。**置換しても顧客向け本文に社内語が残ったら、`GenerateProposalHtml` が usage_log へ `proposal_taboo_left` / `n=<件数>` と V-S5-12 の警告文を残す**（裁定書40 S-M1。一般語とサ変語幹を置換対象から外した代償であり、警告なので生成は止めない）。S5 の経路（`modPipeline5.SoftenOrFail`）では扱いが違い、**置換後の `CheckS5` に1件でも発火が残れば失敗**である（16章 E-71③「通れば成功として続行する」。裁定書40 S-M4）。
...
- `tools/render_proposal.py`（`render_report.py` と同型。gate.py への登録は司令塔が行う）。mock の S5 JSON（`MK-S5`）と S2 JSON（`MK-S2-RNW`）から実物のHTMLを組み、次を機械で見る。
  ...
  14. 対訳表（§1〜§3 の番号付き46語＋§4.1 の代替語3語＋§6.4 の表記ゆれ2語＝**51対**）の社内語・顧客語・印が、`modValidate4.TabooPairs()` の実物と**1行ずつ一致**する（`check_glossary_impl`）
  15. LibreOffice で**実物の `modValidate4.SoftenTaboo` を回し**、51対 × 代表文脈（素・助詞・サ変・活用語尾・末尾の重なり）の**実効出力**が、対訳表§6〜§6.6 の宣言から組み立てた参照実装の出力と1件ずつ一致する。2回通しても変わらない（冪等）ことも同時に見る（`check_glossary_effective`）
- 14 と 15 は W15 最終是正（裁定書42 §1）で足した。それまでは 11 の「対訳表 ⇔ 15章」1辺しか見ておらず、**実際に顧客向け本文を書き換える `TabooPairs()` を壊しても全ゲートが緑**だった（統合レビューが「料率 → ZZZ壊れた顧客語」で実証）。値源（対訳表 Markdown）と実装（VBA）を別々に辿るので、どちらか一方だけが変わると必ず赤くなる。要点行には**実際に検査した対の数と実効出力の件数**を出す。
- 純テスト `src/test/modTestsPure27.bas` と `src/test/modTestsPure28.bas`（層(a)）。28 は 27 が30,000字の上限に達したための続きで、W15 最終是正（裁定書42 §1）の**17本**——用言の連用形6本（対訳表§6.3。「仕分けている」を「整理ている」にしない／名詞の位置と「など」「〜する」は従来どおり置換する）・表記ゆれ5本（§6.4。「引受けている」を「保険のお引き受けけている」にしない／「引き受ける」は素通し／「座組み」）・末尾の重なり4本（§6.6。D&O保険／BCP計画／MFA認証と、1文字は吸収しない境界）・対訳表の行数と印の数・吸収と見送りを混ぜた冪等——を持つ。27 は**合計61本**（G1=14 / G2=5 / G3=4 / G4=3 / G5=16 / G6=19。本数は `wintest/tests_expected.txt` の `prod` と必ず同時に直す）。CheckS5 の13件・登録表の枚数・空データで落ちないこと・内部値がDATAに入らないこと・`reviewedBy` 空で生成しないこと・ファイル名規則。W15 Round 2 で16本を足した（裁定書39）: 対訳表の最長一致4本（壊す班が実測した「未保険のご加入」「保険のご加入ギャップ」「リスク保険で備える可能性」「ご提案の構成パターン」を逐語で禁じる）・一般語を置換しないこと2本・一般語でも V-S5-12 は出ること・冪等・英字の語境界（CBI を BI で刻まない）・機械置換の件数を呼出側が受け取ること・描画例外の印・`?debug=1` の赤枠・必須キー検査2本・空白類だけの確認者名を認めないこと2本。W15 Round 2 の検収でさらに19本を足した（裁定書40）: 会社名の TAB で表紙がずれないこと2本・残った社内語を呼出側へ知らせること3本・サ変語幹4本（10語すべてを「〜する」「〜し」「〜され」で確かめ、名詞の位置では置換することも見る）・`TabooHitStrict` の両方向2本・語境界（BIG/OTC/SLAB と単独の BI/OT）2本・連鎖する対の実効出力1本・置換後の CheckS5 の見分け3本・確認者名の NBSP/VT/FF 2本。
