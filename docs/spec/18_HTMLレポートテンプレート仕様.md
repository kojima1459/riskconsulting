# 18. HTMLレポートテンプレート仕様 v1.7

v1.7（W15 Round 3・裁定書41 §2）: 2点を**実装に合わせて**訂正した。**(1) §2 `meta.s1_warn`**: 運ぶケースIDが V-S1-14/15 の2件ではなく **V-S1-14/15/16/17 の4件**であること（裁定書40 P-M1 で V-S1-16/17 を注記チャネルへ移した事実に追随していなかった）。**(2) §3 SEC-04**: `missing_info[].kind` の欠落・空は `modValidate3.PostNormalize` が `"not_found"` を**補填済み**なので「種別」列が空欄にならないこと。

v1.6（W15 Round 2・裁定書39 班Q）: 3点を改訂した。**(1) §3 SEC-09 と §2 `meta.ground_unmatched`**: `modGround.GroundNotes` が `emerging_risks[]` を `"E1"` `"E2"` と採番して返しているのに、それを読む描画側が**どこにも無かった**（SEC-14 は `risks[]` しか描かない）ので、SEC-09 のカードへ「原文未照合」の1行を足し、`ground_unmatched` を読む場所を SEC-14 と SEC-09 の2箇所と明記した（R1-08）。**(2) §5.2 `coverFields` の安全根拠を事実へ訂正**した。タブ区切りが成立するのは `SanitizeInput` が制御文字を落とすからではなく（同関数はタブとLFを意図的に**残す**）、**連結側が各フィールドからタブ・改行を落とすから**である。旧記述のままでは確認者名にタブを1つ入れるだけで `<noscript>` の確認日時を偽装できた（R2-05）。**(3) §2 `meta.reviewed_by` の空判定**を「空白類（半角／全角／タブ／LF／CR）を除いて1文字以上」と明記した（R2-04）。
...
- `meta.reviewed_by` / `meta.reviewed_at` は**担当者が内容を確認・編集したか**（v1.4・裁定書37 B-06）。未確認は**両方とも空文字**（キー自体は必ず置く）。値を入れるのは `modExportHtml.GenerateHtmlReportEx` の `reviewedBy` 引数が**空白類を除いて1文字以上**のときだけで（v1.6・裁定書39 R2-04。判定は `modUtilText.HasVisibleText` の1本＝レポートと提案書が同じ1本を呼ぶ。VBAの `Trim$` は半角空白しか落とさないため、旧判定 `LenB(Trim$(x))>0` はタブ・LF・CR・全角空白だけの名前を「確認済み」と認めていた。正規化の口は `modExportHtml.ReviewerOf`）、`reviewed_at` はそのとき生成側が打つ（`modUtil.NowStamp` の `yyyy/mm/dd hh:mm:ss`）。読むのは §3.5 の免責1行目と §3 SEC-01 の表紙チップ、および §4.1 の `<noscript>`。
- `meta.ground_unmatched` は**引用の原文照合で見つけられなかったリスクの番号**の文字列配列（v1.4・裁定書37 B-03。0本以上。キー自体は必ず置き、無ければ空配列）。`s2.risks[]` は `risk_no` をそのまま、`s2.emerging_risks[]` は `risk_no` を持たないため配列の出現順に `"E1"` `"E2"` と採番した値が入る。値源は `modGround.GroundNotes`（貼付原文＝`case_data` の `input_*` 全欄＋`s1_json` と突き合わせる純関数）で、`evidence.source` が `inference` / `knowledge` のものと、貼付原文が空のときは**検査しない**（空配列になる。config `ground_check` / `ground_head_chars`＝13章§2.3）。読むのは §3 SEC-14 の「原文照合」列（`risk_no` で引く）と **§3 SEC-09 のカード内の「原文未照合」の1行（`"E"+(配列index+1)` で引く）** の2箇所であり（v1.6・裁定書39 R1-08。SEC-14 は `risks[]` しか描かないため、`emerging_risks[]` の未照合が `E1` `E2` として `ground_unmatched` に入っていても**どこにも出ない**状態だった。捏造が最も出やすいのは一般論を書きやすい `emerging_risks[]` なので、採番した側の節に印を出す）、**SEC-04 の充足度バッジは動かさない**（別の事実なので混ぜない）。
- `meta.s1_warn` は**S1の「落とさない警告」4件の点検結果**の文字列配列（v1.5・裁定書38 班A。**v1.7・裁定書40 P-M1 で運ぶケースIDが4件になった**: V-S1-14〔`sources[].url` が貼付原文に実在しない〕/ V-S1-15〔「(見立て)」の欠落・`financials` の出所と値の食い違い〕/ V-S1-16〔`sources` の欠落〕/ V-S1-17〔`missing_info[].kind` が enum 外〕。16/17 は以前 `modValidate.CheckS1` の**不合格**側に居たが、それでは修復リトライ→案件 status=error を起こすので注記チャネルへ移した。0本以上。キー自体は必ず置き、無ければ空配列）。要素は `"V-S1-14:2"` のように**ケースIDと件数**だけで、本文（どのURLか）は載せない（レポートは障害報告書ではない＝16章NFR-S3。本文は run_log の detail にある）。値源は `modValidate3.CheckS1Notes`→`WarnNoteOf`（貼付原文＝`case_data` の `input_*` 全欄だけと突き合わせる。S1出力は混ぜない）。読むのは §3 SEC-14 の末尾の注記行だけで、**SEC-04 の充足度バッジは動かさない**（別の事実なので混ぜない）。
...
| SEC-04 | sufficiency | 入力の充足度と要確認事項 | `s1.input_quality.coverage[]`（`aspect` `status`） / `s1.input_quality.overall` / `s1.input_quality.advice` / `s1.missing_info[]`（`item` `why_needed` `kind`） | `s1` が null なら非表示 | **最上段に `kind="conflict"` の分離表示**＋14観点バッジ＋表（表は `conflict` 以外を出し、「種別」列に `kind` の日本語ラベル〔19章§3〕を添える。v1.5・裁定書38 B-11。**`kind` が欠落・空のときは `modValidate3.PostNormalize` が `"not_found"` を補填済み**なので、この列が空欄になることはない〔v1.7・裁定書41 §2。補填が無いと日本語ラベルの引き当てに失敗して空欄で出る〕） |
...
| SEC-09 | newrisk | 新しく出てきたリスク（新種・新興リスク） | `s2.emerging_risks[]`（`risk_name` `category` `horizon` `scenario` `evidence_quote` `evidence_source` `proposal_hint`）＋ **`meta.ground_unmatched`**（v1.6・裁定書39 R1-08。配列の i 番目のカードは `"E"+(i+1)` で引く。`modGround.GroundNotes` の採番と同じ数え方＝出現順の1始まり） | 0件（空配列）のときは「現時点で特筆すべき新しく出てきたリスクは検出されていません」の1行を出す（**非表示にしない**。「見ていない」のではなく「見たうえで該当が無い」ことを読み手に示すため） | カード。`ground_unmatched` に当該Eが在るカードには根拠の直後に「原文未照合: 貼り付けた資料の中にこの根拠を見つけられませんでした（表記の違いで見つからないこともあります）。」の1行を足す（SEC-14 の「原文未照合」と同じ語を使う） |
...
- **`coverFields` の書式**: §4.1(b)の3値（会社名／案件ID／生成日時）を**タブ（`vbTab`）区切りの1本の文字列**で渡す。`[0]`=会社名 `[1]`=案件ID `[2]`=生成日時。タブが区切りとして安全なのは、**連結する側（`modExportHtml.BuildReportHtml`）が各フィールドから `vbTab` / `vbLf` / `vbCr` を落としてから並べるため**である（v1.6・裁定書39 R2-05。旧記述「外部由来テキストが `modUtilText.SanitizeInput`〔16章 E-04〕で制御文字を落としてから案件データに入るため」は**事実と異なっていた**: `SanitizeInput` はタブとLFを『本文の構造』として**意図的に残す**ので、確認者名にタブを1つ入れるだけで `FieldAt` の返す値が1つずつずれ、`<noscript>` の確認日時〔`[4]`〕を任意の文字列へ差し替えられた）。区切りを守る責任は**連結側**にあり、テンプレ側は `FieldAt` で位置を数えるだけである。3値をJSONで渡さないのは、テンプレ側にJSONパーサを持たせない（＝純文字列モジュールに留める）ため。**この3値はテンプレ側で `HtmlSafe` を通す**（§4.1・§5.3(2)）。
