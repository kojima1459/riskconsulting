...
姉妹PJの開発セッション（**`claude/internal-notebook-lm-chatbot-B6BE7`ブランチ・45コミット・chatbot_v2系**）に§1〜3を照会したところ「4問中3問は前提不成立（誤報）」と返ってきたが、**実物照合の結果、これはブランチ相違**であって本書の誤りではない。同一リポジトリに系譜の異なるブランチが並存している。
