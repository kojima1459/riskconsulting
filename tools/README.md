...
- 検問を1つでも飛ばした版は配布しない。
- **要点行は「実際に検査した件数」を名乗る**(W15 §3 X3-3。`tools/gate_count.py` 参照)。
  固定の作文で「確認しました」と書かない。
...
### `notice_check.py` - 注記の出し方の回帰検問(裁定書40 Q-M2 / 裁定書41 §2 で gate 登録)

```bash
python3 tools/notice_check.py
python3 tools/notice_check.py --only dom     # 検査①だけ
python3 tools/notice_check.py --only wire    # 検査②だけ
# exit code: 0 = OK / 1 = 検査NG / 2 = 検査を実行できない(node 無し等)
```

純層テストが原理的に届かない2点だけを受け持つ。検査①は LibreOffice に**実物のHTML**を
組ませ、node の最小DOMスタブで**ページ内JSを実際に走らせて** SEC-09「原文未照合」の印の
位置を両方向(空のとき出ない/指定したカードだけに出る)で測る(SEC-14 の「原文照合」列へも
横展開)。検査②は 16章 E-02 の警告帯が **DrawStep → ShowStepNotice** まで実際に配線されて
いるかを手続き単位で読む。**node が無ければ 0 を返さない**(exit 2。「実行できなかった」を
「合格」に化けさせない)。`--only` で片方だけ回した回は、要点行に回した検査の名前しか
出ない(`gate_count` の契約)。

### `gate_count.py` - 検問の要点行の契約(W15 §3 X3-3。「検査していないのに緑」を止める)

```bash
python3 tools/gate_count.py            # 登記の網羅 + 自己テスト
python3 tools/gate_count.py --selftest
# exit code: 0 = OK / 1 = 契約違反 / 2 = 自己テスト失敗
```

`gate.py` は **returncode だけ**で赤緑を決め、要点行は表示専用である。そのため各ツールの
成功文言が固定の作文だと、**飛ばした検査まで「確認しました」と名乗れる**(実測: node 不在で
DOM検査を丸ごと飛ばしたのに「描画後DOM…を確認しました」で exit 0、`notice_check --only dom`
で検査②が「(未実行)」のまま OK 行、など)。そこで3点セットを共通化した。

1. **数える** `gate_count.Checked().record(名前, 件数)` … **実際に回した検査だけ**を積む。
2. **名乗る** `gate_count.report(checked, required=…)` … `検査実施: 合計件 (名前=件, …)` を
   1行出す。飛ばした検査は**名前ごと消える**ので固定の作文ができない。
3. **落ちる** … 合計0件、または `required` の検査が0件なら非0を返す(fail-closed)。

さらに本ツールを単体で走らせると、`gate.py` の `GATES` に載る**全スクリプト**が
「契約適用(`CONTRACT_TOOLS`)」か「理由付きの未適用登記(`PENDING_TOOLS`)」のどちらかで
あることを確かめる。どちらにも無いスクリプトが増えたら赤、GATES から消えたのに登記だけ
残っていても赤(同じゲート名の二重登録も赤)。**新しい検問が黙って契約の外へ出るのを止める**
のがこの登記の役目で、`vba_lint.py` の `CONTRACT` と同じ考え方である。

適用済み: `orphan_check.py` `doc_gate.py` `render_report.py` `notice_check.py`。
`render_proposal.py` は裁定書42 §3.4 により班X1/X2 の担当なので未適用登記(handoff)。
...
自モジュール内の `SRC & ".名前"` というログの名札は使用に数えない)②build/ tools/ の
テキストの**実行される部分** ③動的連結の救済 のいずれにも出現しない識別子を孤児として
報告する。定義直前5行以内の `' @unused:理由` でSKIP。
**docs/ は救済しない**(仕様書に名前を書くことは使うことではない。docs/ にしか名前が
無いものは `ERROR(docs-only)` として赤にする)。

- **②の線引き(W15 §3 X3-2)**: `build/` `tools/` でも「実行されない散文」では救済
  しない。落とすのは .py のコメントと docstring、.md の地の文(コードブロックと
  インラインコードだけを残す)、.ps1/.bat/.yml/.ini/.cfg の行コメント。
  .json/.csv/.txt はデータなのでそのまま。`tools/vba_lint.py` の
  `CONTRACT`/`MODULE_REGISTRY` と `build/modules.json` は「登記」なので丸ごと対象外
  (`REGISTRY_ONLY_TEXT`)。
- **③の線引き(W15 §3 X3-1)**: 救済は「宛先モジュール」と「接尾辞の形」の**両方**で
  閉じる。以前は接頭辞の前方一致だけだったので、src のどこかに `"ShowArea" & 式` が
  1つあれば**無関係なモジュール**の `ShowArea…` がすべて永久に救済された(統合レビューが
  実測で再現)。いまは
  `"モジュール名.接頭辞" & 式` → 宛先はそのモジュール /
  `"接頭辞" & 式` → 宛先は**その連結を書いているモジュール自身** /
  `モジュール名.HandlerName("接頭辞", …)` → 宛先はそのモジュール、
  接尾辞は 連番型(`& CStr(n)` 等)なら**数字だけ**、名前組立型なら宛先/連結元モジュールの
  文字列リテラルにある snake_case 語のパスカル形に**完全一致**、でなければ救済しない。
- **自己言及の罠**: このツール自身のdocstring・コメント・**テストの文字列**に孤児候補の
  識別子を直書きすると、tools/ を読む参照集合(②)がその識別子を「参照されている」と
  誤認し、二度と検出できなくなる(実測で2度発見。合成名は `ZzT` 接頭辞で書くこと)。
